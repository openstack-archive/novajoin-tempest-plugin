...
tempest config files. To work around this, comment out the line:
...
The tests in test_tripleo_deployment should be run on the undercloud in a TLS
enabled tripleo deployment.  These tests verify that the undercloud and overcloud
nodes are registered with IPA and that the required hosts and services have been
created in the IPA server.  In addition, it confirms that the certificates
requested by Heat are tracked by certmonger on the overcloud nodes.
